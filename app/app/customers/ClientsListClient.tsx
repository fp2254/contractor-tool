...
import TradeContactsClient from "@/app/app/trade-contacts/TradeContactsClient";
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-3">
        <h1 className="text-xl font-bold text-slate-800">
          {activeTab === "clients" ? "Clients" : "Trade Contacts"}
        </h1>
        {activeTab === "clients" ? (
          <button
      ...
      {/* Tab toggle */}
      <div className="grid grid-cols-2 gap-1 bg-gray-100 rounded-2xl p-1 mb-4">
        <button
          onClick={() => { setActiveTab("clients"); setMode("idle"); setQ(""); }}
          ...
          {active.length > 0 && (
            <span className={`ml-1.5 text-[11px] font-bold px-1.5 py-0.5 rounded-full ${
              activeTab === "clients" ? "bg-gray-100 text-gray-600" : "bg-gray-200 text-gray-500"
            }`}>
              {active.length}
            </span>
          )}
        ...
        <button
          onClick={() => { setActiveTab("trade"); setMode("idle"); setQ(""); }}
          className={`rounded-xl py-2.5 text-sm font-semibold transition-all ${
            activeTab === "trade"
              ? "bg-white text-slate-800 shadow-sm"
              : "text-gray-500 hover:text-gray-700"
          ...
          🔧 Trade Contacts
          {tradeContacts.filter(c => !c.archived).length > 0 && (
            <span className={`ml-1.5 text-[11px] font-bold px-1.5 py-0.5 rounded-full ${
              activeTab === "trade" ? "bg-gray-100 text-gray-600" : "bg-gray-200 text-gray-500"
            }`}>
              {tradeContacts.filter(c => !c.archived).length}
            </span>
          )}
        </button>
      </div>
      ...
      {/* ── Trade Contacts tab ── */}
      {activeTab === "trade" && (
        <TradeContactsClient initialContacts={tradeContacts} />
      )}
    </>
  );
}
