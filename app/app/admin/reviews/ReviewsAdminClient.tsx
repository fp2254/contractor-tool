...
export default function ReviewsAdminClient({ initialRows }: { initialRows: ReviewRow[] }) {
  ...
  const filtered = rows.filter((r) => {
    ...
    if (q.trim()) {
      ...
      if (!r.reviewer_name.toLowerCase().includes(s) && !r.org_name.toLowerCase().includes(s) && !r.text.toLowerCase().includes(s)) return false;
    }
    return true;
  });
    ...
    <div className="space-y-3">
      <div className="flex gap-2 flex-wrap">
        ...
        <div className="flex gap-1 bg-white rounded-xl border border-gray-200 p-1">
          {[["all", "All"], ["approved", "Approved"], ["hidden", "Pending"]].map(([v, label]) => (
              ...
              <div key={r.id}>
                <div
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-600 mt-0.5 line-clamp-1">{r.text}</p>
                  </div>
                ...
                {expanded === r.id && (
                  <div className="px-4 pb-4 bg-gray-50 border-t border-gray-100 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-gray-500">
                      ...
                      <div><span className="font-semibold">Location:</span><br />{r.location ?? "—"}</div>
                    ...
                    <div className="flex gap-2 pt-1">
                      <button
                        ...
                        {toggling === r.id ? "…" : r.approved ? "Unapprove" : "Approve"}
                      </button>
                      <button
