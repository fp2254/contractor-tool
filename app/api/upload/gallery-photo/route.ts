...
export async function POST(req: Request) {
  ...
  const path = `${orgId}/gallery_${Date.now()}.${ext}`;
  ...
  if (uploadError) {
    console.error("[gallery-photo-upload] storage error:", uploadError.message);
    return NextResponse.json({ error: uploadError.message }, { status: 500 });
  }

  const { data: { publicUrl } } = admin.storage.from("profile-photos").getPublicUrl(path);

  return NextResponse.json({ url: publicUrl, title });
}
