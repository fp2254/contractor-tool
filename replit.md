...
- **Database Schema**: Utilizes `profiles`, `clients`, `invoices`, `invoice_items`, `invoice_attachments`, `quotes`, `quote_items`, and `referral_earnings` tables. Specific fields for subscription status, payment tracking, and quote management are critical.
- **File Storage**: Supabase Storage is used for `logos` and `invoice-photos`.
- **API Endpoints**: Comprehensive set of RESTful APIs for managing profiles, clients, invoices, quotes, payments, referrals, and Stripe integrations.
