...
- **Nov 25, 2025**: Fixed translation bugs in English language section - replaced French text that was incorrectly placed in English translations (invoice line items, cost helper, materials, markup, placeholders). Added missing English and Spanish inventory placeholder translations. Added `applyLanguage()` call to `showScreen()` function to ensure translations update when switching screens. Updated service worker cache to v5 to force refresh.
